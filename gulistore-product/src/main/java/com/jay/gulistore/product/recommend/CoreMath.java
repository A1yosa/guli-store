...
public class CoreMath {
    ...
    public List<Integer> recommend(Integer userId, List<RelateDTO> list) {
        ...
        Map<Integer, List<RelateDTO>> userMap =list.stream().collect(Collectors.groupingBy(RelateDTO::getId));

        //最近邻用户买过的商品id列表
        List<Integer> neighborItemList = userMap.get(nearest).stream().map(e->e.getSkuId()).collect(Collectors.toList());
        //指定用户买过的商品id列表
        List<Integer> userItemList  = userMap.get(userId).stream().map(e->e.getSkuId()).collect(Collectors.toList());
    ...
    private double pearson_dis(List<RelateDTO> xList, List<RelateDTO> yList) {
        ...
        xList.forEach(x->{
            yList.forEach(y->{
                if(x.getSkuId().intValue() == y.getSkuId().intValue()){
                    xs.add(x.getStar());
                    ys.add(y.getStar());
                }
            });
